...
import { Review, ClimbingGym } from './types';
...
export default function App() {
  ...
  const handleNavigation = (tab: 'home' | 'map' | 'saved' | 'profile') => {
    setCurrentState(tab);
    if (tab !== 'gym-detail') {
  ...
  const handleSaveToggle = async (gymId: string) => {
    try {
      ...
      const updatedSavedIds = await savedGymsApi.toggleSaveGym(deviceId, gymId, action);
  ...
  const handleReviewLike = async (reviewId: string) => {
    ...
    try {
      const updatedReview = await reviewsApi.likeReview(reviewId, selectedGymId);
      setReviews(prev =>
        prev.map(review =>
  ...
  const handleNewReview = async (newReviewData: {
    ...
    try {
      const newReview = await reviewsApi.createReview(selectedGymId, newReviewData);
  ...
  const selectedGym = selectedGymId ? gyms.find(gym => gym.id === selectedGymId) : null;
  const gymReviews = selectedGymId ? reviews.filter(review => review.gymId === selectedGymId) : [];
