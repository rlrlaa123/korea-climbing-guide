...
app.get('/make-server-1d6c2b3c/gyms/:gymId/reviews', async (c) => {
  try {
    ...
    return c.json({ 
      success: true, 
      reviews: reviews.map(r => r.value).sort((a, b) => 
...
app.post('/make-server-1d6c2b3c/gyms/:gymId/reviews', async (c) => {
  try {
    ...
    if (!reviewData.nickname || !reviewData.country || !reviewData.rating || !reviewData.text) {
...
app.post('/make-server-1d6c2b3c/saved-gyms/:deviceId', async (c) => {
  try {
    ...
    return c.json({ success: false, error: 'Failed to update saved gyms' }, 500)
  }
})
